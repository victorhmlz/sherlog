 ...
 * `score`/`scoreBreakdown` (TASK 06), `risk` (TASK 07), and
 * `signal`/`signalReason` (TASK 08) are then recomputed from those
 * updated values, in that order (signal reads score+risk), so the
 * dashboard's score, tier, sort-order, risk badges, and signal badge
 * all genuinely track the live feed instead of freezing — or worse,
 * silently disagreeing with each other — while everything around them
 * moves.
 *
 * Deliberately NOT touched here: `holders` / `top10Concentration`
 * (holder analysis — TASK 14), and `auction` (Long adapter —
 * TASK 16/17). Ticking those would fake future engines' output, not
 * simulate a market feed — the Score/Risk Engines still read their
 * existing (static) mock values as inputs for the components that
 * depend on them (`holders`/`auction` for score, `concentration` for
 * risk).
 */
export function tickToken(token, { elapsedMs = MOCK_STREAM_INTERVAL_MS } = {}) {
  ...
  const risk = computeRisk(nextToken);
  const withScoreAndRisk = { ...nextToken, score, scoreBreakdown: breakdown, risk };
