 ...
 * `score`/`scoreBreakdown` (TASK 06) and `risk` (TASK 07) are then
 * recomputed from those updated values, so the dashboard's score,
 * tier, sort-order, and risk badges genuinely track the live feed
 * instead of freezing while everything around them moves.
 *
...
export function tickToken(token, { elapsedMs = MOCK_STREAM_INTERVAL_MS } = {}) {
  ...
  return { ...nextToken, score, scoreBreakdown: breakdown, risk };
}
