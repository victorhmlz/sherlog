 ...
 *
 * `isBuyable` (TASK 10): a convenience boolean so "which tokens looked
 * buyable on a given day" doesn't require remembering/replicating the
 * `setup` threshold in every query. Computed once, at capture time, as
 * `setup IN ('SETUP A', 'EXTREME')` — see
 * `lib/data/capture.js:isBuyableSetup`. This is a stored judgment call
 * with an explicit, adjustable definition, not a new independent
 * signal — `setup` remains the source of truth if the threshold ever
 * needs to change (SETUP B could reasonably be included too; V1 keeps
 * it narrow, matching the Signal Engine's own "SETUP A+ needs BOTH
 * volume AND buyer acceleration for EXTREME" discipline of erring
 * strict rather than loose). `.default(false)` exists purely so this
 * column can be added safely via `ALTER TABLE` to a table that may
 * already have rows — every row this app itself ever inserts always
 * supplies an explicit value (see `capture.js`); the default only
 * matters for the migration, never for application logic.
 */
export const signals = pgTable(
  "signals",
  {
    ...
    isBuyable: boolean("is_buyable").notNull().default(false),
